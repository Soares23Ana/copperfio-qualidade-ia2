...
class _ClientChatHomePageState extends State<ClientChatHomePage> {
  ...
  void initState() {
    ...
    _messages.add({
      'sender': 'bot',
      'text':
          'Olá! Eu sou o assistente virtual Copper. Posso ajudar com orçamento, pedido, chamado ou dúvidas sobre produtos.',
    });
  }
  ...
  String _getBotResponse(String text) {
    ...
    if (message.contains('problema') ||
        message.contains('chamado') ||
        message.contains('suporte')) {
      return 'Entendi. Posso ajudar na criação do seu chamado e esclarecer o processo.';
    }
    if (message.contains('produto') ||
        message.contains('cabo') ||
        message.contains('alumínio')) {
      return 'Tenho informações sobre nossos cabos e fios de alumínio. Qual é a sua dúvida específica?';
    }
    if (message.contains('falar com') ||
        message.contains('alguém') ||
        message.contains('atendente') ||
